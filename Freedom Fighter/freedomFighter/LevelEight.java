...
public class LevelEight implements Level {
	...
	public LevelEight() {
		...
		bgMusicSongs[0] = "Sound\\russia.wav";
		...
		try {
			background = GameDriver.addImage("background\\Baikonur.jpg");
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void spawnStage2() {
		if (timer1 >= minute / 2 && timer2 < minute / 2) {
			timer2++;
			if (aliens.size() == 0) {
				aliens.add(new RayDestroyer(28, -500, 3));
				aliens.add(new RayDestroyer(181, -500, 3));
				aliens.add(new RayDestroyer(334, -500, 3));
				aliens.add(new RayDestroyer(28, -300, 3));
				aliens.add(new RayDestroyer(181, -300, 3));
				aliens.add(new RayDestroyer(334, -300, 3));
				
				aliens.add(new Cruiser(63, -400, 1));
				aliens.add(new Cruiser(377, -400, 1));
			}
		}
	}
