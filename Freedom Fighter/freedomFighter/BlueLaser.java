...
public class BlueLaser implements Projectile{
	private Hitbox orb;
	private int dy, dx;
	...
	public BlueLaser (int x, int y) {
		try {
			imgPad = GameDriver.addImage("projectiles\\Alien Laser (Blue).png");
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	public BlueLaser(int x, int y, double theta) {
		...
		try {
			if (theta == Math.PI/4) imgPad = GameDriver.addImage("projectiles\\Alien Laser (Blue) Left.png");
			else imgPad = GameDriver.addImage("projectiles\\Alien Laser (Blue) Right.png");
		}
		catch (Exception e) {
			e.printStackTrace();
		}
