...
public class Player {
	...
	public Player (int x, int y, BufferedImage aimg) {
		imgPad = aimg;
	...
	public void moveAndDraw(Graphics2D g2, boolean isUp, boolean isDown, boolean isLeft, boolean isRight) {
		...
		if(isDown && ship.getY()+imgPad.getHeight() <= 720) {
			ship.translate(0, dy);
		}
        if(isLeft && ship.getX() >= 20) {
        	ship.translate(-dx,0);
        }
        if(isRight && ship.getX()+imgPad.getWidth() <= 620) {
        	ship.translate(dx,0);
        }
        g2.drawImage(imgPad, null, (int)ship.getX(), (int)ship.getY());
        if (shield > 0) {
        	if (!hit) {
