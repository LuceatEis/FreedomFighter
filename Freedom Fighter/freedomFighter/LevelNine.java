...
public class LevelNine implements Level {
	...
	public LevelNine() {
		...
		try {
			background = GameDriver.addImage("background\\Moscow.jpg");
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void spawnStage2() {
		if (timer1 >= minute / 2 && timer2 < minute / 2) {
			...
			if (aliens.size() == 0) {
				...
				aliens.add(new Battlecruiser(195, -400, 1));
			}
		}
	}
	public void spawnStage3() {
		if (timer2 >= minute / 2 && timer3 < minute) {
			...
			if (aliens.size() == 0) {
				...
				aliens.add(new Battlecruiser(78, -400, 2));
				aliens.add(new Battlecruiser(292, -400, 2));
			}
		}
	}
