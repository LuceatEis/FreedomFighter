...
public class LevelOne implements Level {
	...
	public LevelOne() {
		...
		try {
			background = GameDriver.addImage("background\\Fortaleza.jpg");
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void draw(Graphics2D g2, boolean[] keys) {
		...
		drawInfoPanel(g2);
		if( (isDefeated() && player.isDoneDying()) || ( (timer2 >= minute / 2) && aliens.size() == 0) ) {
			completion = true;
			bgMusic.stop();
			bgMusic = null;
		}
	...
	public void spawnStage1() {
		if (timer1 < minute / 2) {
			timer1++;
			if (aliens.size() == 0) {
				aliens.add(new Corvette(-150, 10, 1));
				aliens.add(new Corvette(-150, 85, 1));
				aliens.add(new Corvette(-150, 160, 1));
	...
	public void spawnStage2() {
		if (timer1 >= minute / 2 && timer2 < minute / 2) {
			timer2++;
			if (aliens.size() == 0) {
				aliens.add(new Corvette(-200, 10, 2));
				aliens.add(new Corvette(-200, 85, 2));
				aliens.add(new Corvette(-200, 160, 2));
				aliens.add(new Corvette(-200, 235, 2));
				aliens.add(new Corvette(-200, 310, 2));
				aliens.add(new Corvette(-200, 385, 2));
				aliens.add(new Corvette(800, 10, 2));
				aliens.add(new Corvette(800, 85, 2));
				aliens.add(new Corvette(800, 160, 2));
				aliens.add(new Corvette(800, 235, 2));
				aliens.add(new Corvette(800, 310, 2));
				aliens.add(new Corvette(800, 385, 2));
			}
		}
	}
